...
# ============== DEBUG LOGGING ==============
DEBUG_STEP_COUNTER = [0]  # Mutable container for step counting

def debug_log_clemens(label, data, max_steps=20):
    """Debug logging helper for Clemens integrator"""
    if DEBUG_STEP_COUNTER[0] >= max_steps:
        return
    ...
    if arr.size > 20:
        print(f"[CLEMENS] {label}: shape={arr.shape}, norm={np.linalg.norm(arr):.6f}, min={arr.min():.6f}, max={arr.max():.6f}")
        # Show last 12 values (actuated joints)
        if arr.size >= 12:
            print(f"[CLEMENS]   last 12: {arr.flatten()[-12:]}")
    else:
        print(f"[CLEMENS] {label}: {arr}")
# ============================================
...
class MoreauIntegrator:
    ...
    def simulate(
        ...
        if DEBUG_STEP_COUNTER[0] < 2:
            ...
            print(f"[CLE] Body transforms (first 3 bodies):")
            for i in range(3):
                print(f"  Body {i}: {body_tf[i]}")
            
            # Print motion subspace vectors
            S_s = state_mid.joint_S_s.numpy()  # or state_aug.joint_S_s
            print(f"[CLE] joint_S_s for FREE JOINT (DOFs 0-5):")
            for i in range(6):
                print(f"  S_s[{i}]: {S_s[i]}")
            
            print(f"[CLE] joint_S_s for FIRST ACTUATED JOINTS (DOFs 6-8):")
            for i in range(6, 9):
                print(f"  S_s[{i}]: {S_s[i]}")
        # ===========================================================
        if DEBUG_STEP_COUNTER[0] < 1:
            print(f"[CLEMENS] Joint-Body Mapping:")
            joint_parent = model.joint_parent.numpy()
            joint_child = model.joint_child.numpy() if hasattr(model, 'joint_child') else None
            joint_names = model.joint_name if hasattr(model, 'joint_name') else None
        ...
        # After self.eval_mass_matrix(model, state_aug)
        if DEBUG_STEP_COUNTER[0] == 0:
            H_np = model.H.numpy()
            L_np = model.L.numpy()
            ...
            print(f"[DEBUG] L-Matrix diag (first 6): {L_np.reshape(-1, 18, 18)[0].diagonal()[:6]}")
                ...
                # ============== DEBUG CHECKPOINT 1 ==============
        if DEBUG_STEP_COUNTER[0] < 20:
            print(f"\n{'='*60}")
            print(f"[CLEMENS] ========== STEP {DEBUG_STEP_COUNTER[0]} ==========")
            print(f"{'='*60}")
            debug_log_clemens("INPUT joint_q", state_in.joint_q)
            debug_log_clemens("INPUT joint_qd", state_in.joint_qd)
            debug_log_clemens("MID joint_q (half-step)", state_mid.joint_q)
            debug_log_clemens("joint_act", model.joint_act)
            debug_log_clemens("joint_target", model.joint_target)
            debug_log_clemens("joint_target_ke", model.joint_target_ke)
            debug_log_clemens("joint_target_kd", model.joint_target_kd)
            debug_log_clemens("joint_limit_ke", model.joint_limit_ke)
            debug_log_clemens("joint_limit_kd", model.joint_limit_kd)
        # ================================================
        # === ACTION ROUTING DEBUG ===
        if DEBUG_STEP_COUNTER[0] < 3:
            q_np = state_mid.joint_q.numpy()
            target_np = model.joint_target.numpy()
            act_np = model.joint_act.numpy()
            ke_np = model.joint_target_ke.numpy()
            # Actuated joints start at coord_start=7 for joint_q/joint_target
            # and dof_start=6 for joint_qd/joint_act
            for i in range(3):  # First 3 actuated joints
                coord_idx = 7 + i  # joint_q/joint_target index
                dof_idx = 6 + i    # joint_qd/joint_act index
                joint_idx = 1 + i  # joint index for ke/kd arrays
                print(f"[CLEM] Joint {i}: q[{coord_idx}]={q_np[coord_idx]:.4f}, target[{coord_idx}]={target_np[coord_idx]:.4f}, act[{dof_idx}]={act_np[dof_idx]:.4f}, ke[{joint_idx}]={ke_np[joint_idx]:.1f}, error={target_np[coord_idx]-q_np[coord_idx]:.4f}")
        ...
        if DEBUG_STEP_COUNTER[0] < 20:
            wp.synchronize_device()
            debug_log_clemens("joint_tau (PRE-CONTACT)", state_mid.joint_tau)
            debug_log_clemens("body_f_s (bias forces)", state_mid.body_f_s)
        # ================================================
        ...
        if DEBUG_STEP_COUNTER[0] < 20:
            wp.synchronize_device()
            debug_log_clemens("percussion (contact impulses)", state_mid.percussion)
            debug_log_clemens("body_f_s (after contact)", state_mid.body_f_s)
        # ================================================
        ...
        if DEBUG_STEP_COUNTER[0] < 20:
            wp.synchronize_device()
            debug_log_clemens("joint_tau (POST-CONTACT)", state_out.joint_tau)
        # ================================================
        ...
        if DEBUG_STEP_COUNTER[0] < 2:
            ...
            print(f"[CLEMENS] H matrix (first 6x6 block):")
            print(H_block[:6, :6])
            print(f"[CLEMENS] L matrix (first 6x6 block):")
            print(L_block[:6, :6])
            print(f"[CLEMENS] H matrix (rows 6-12, cols 0-6) - coupling terms:")
            print(H_block[6:12, :6])
            print(f"[CLEMENS] L matrix (rows 6-12, cols 0-6) - coupling terms:")
        ...
        if DEBUG_STEP_COUNTER[0] < 20:
            wp.synchronize_device()
            debug_log_clemens("joint_qdd (accelerations)", state_out.joint_qdd)
        # ================================================
        ...
        if DEBUG_STEP_COUNTER[0] < 5: # Toggle this manually if needed
            wp.synchronize_device()
        ...
        if DEBUG_STEP_COUNTER[0] < 20:
            wp.synchronize_device()
            debug_log_clemens("OUTPUT joint_q", state_out.joint_q)
            debug_log_clemens("OUTPUT joint_qd", state_out.joint_qd)
            print(f"[CLEMENS] ========== END STEP {DEBUG_STEP_COUNTER[0]} ==========\n")
            DEBUG_STEP_COUNTER[0] += 1
        # ================================================

        # ================== COMPREHENSIVE STATE DEBUG ==================
        # Place this at the VERY END of the simulate() method
        if DEBUG_STEP_COUNTER[0] < 5: # Toggle this manually if needed
            wp.synchronize_device()
            
            
            # 1. Get Generalized Coordinates (The Source of Truth)
            jq = state_out.joint_q.numpy()
            jqd = state_out.joint_qd.numpy()
            
            # 2. Get Cartesian Body States (The Derived View - Used by Policy/Rewards)
            bq = state_out.body_q.numpy()
            bqd = state_out.body_qd.numpy()
            
            print(f"\n[SIMULATE END CLE] Step Output Analysis ==========================")
            
            # --- CHECK BASE (Body 0) ---
            # Generalized: [px, py, pz, qx, qy, qz, qw]
            base_gen_pos = jq[0:3]
            base_gen_rot = jq[3:7]
            base_gen_vel_lin = jqd[3:6]
            base_gen_vel_ang = jqd[0:3]
            
            # Cartesian: Body 0
            base_cart_pos = bq[0][0:3]
            base_cart_rot = bq[0][3:7]
            # Spatial Vel: [wx, wy, wz, vx, vy, vz]
            base_cart_vel_ang = bqd[0][0:3]
            base_cart_vel_lin = bqd[0][3:6]
            
            print(f"BASE POS | Gen: {base_gen_pos} | Cart: {base_cart_pos}")
            print(f"BASE ROT | Gen: {base_gen_rot} | Cart: {base_cart_rot}")
            print(f"BASE VEL | Gen: {base_gen_vel_lin} | Cart: {base_cart_vel_lin}")
            
            # Check for Mismatch (Drift or Missing FK)
            pos_diff = np.linalg.norm(base_gen_pos - base_cart_pos)
            if pos_diff > 1e-4:
                print(f"⚠️ CRITICAL MISMATCH: Body_Q does not match Joint_Q (Diff: {pos_diff:.6f})")
                print(f"   -> Forward Kinematics (FK) likely NOT run on state_out!")
            else:
                print(f"✅ CONSISTENT: Body_Q matches Joint_Q")

            # --- CHECK JOINT 1 (First Actuated Joint) ---
            # Assuming Joint 1 maps to a specific body (e.g. Body 10 in NEW)
            # We check if that body has moved from rest
            
            # Get Joint Index 7 (First actuated DOF after 7-dof base)
            joint_val = jq[7] 
            print(f"JOINT 7  | Val: {joint_val:.6f}")
            
            # Check Body Poses (Are they all zero/identity?)
            bodies_active = 0
            for i in range(len(bq)):
                if np.linalg.norm(bq[i][0:3]) > 1e-6:
                    bodies_active += 1
            
            print(f"BODIES   | {bodies_active}/{len(bq)} bodies have non-zero position")
            if bodies_active <= 1:
                print(f"⚠️ WARNING: Only base (or no bodies) moved. FK issue?")
    ...
    def eval_mass_matrix(self, model, state_mid):
        ...
        if DEBUG_STEP_COUNTER[0] < 1:
            ...
            J_np = model.J.numpy()[:num_bodies*6*num_dofs].reshape(num_bodies*6, num_dofs)
            M_np = model.M.numpy()[:num_bodies*6*num_bodies*6].reshape(num_bodies*6, num_bodies*6)
            body_I_s_np = state_mid.body_I_s.numpy()
            
            print(f"\n[CLEMENS] ===== J-M CONSISTENCY CHECK =====")
            
            # For each joint, show which body row it fills in J
            print(f"[CLEMENS] Joint-to-Body mapping in J construction:")
            joint_child = model.joint_child.numpy() if hasattr(model, 'joint_child') else None
            for i in range(num_bodies):
                ...
                child = joint_child[i] if joint_child is not None else "N/A"
                print(f"  J rows {i*6}-{i*6+5} (joint {i}, child={child}): non-zero DOFs = {non_zero_dofs.tolist()}")
            
            # Show M diagonal block masses
            print(f"\n[CLEMENS] M diagonal block masses (body_I_s[i][3,3]):")
            for i in range(num_bodies):
                mass_from_M = M_np[i*6+3, i*6+3]  # Mass is at [3,3] of spatial inertia block
                mass_from_I_s = body_I_s_np[i][3, 3]
                print(f"  M block {i}: mass={mass_from_M:.4f}, body_I_s[{i}] mass={mass_from_I_s:.4f}")
            
            # Show first few rows of J and M
            print(f"\n[CLEMENS] J matrix (rows 0-17, cols 0-5):")
            print(J_np[:18, :6])
            
            print(f"\n[CLEMENS] M matrix diagonal (6x6 blocks, showing [3,3] element = mass):")
            for i in range(num_bodies):
                print(f"  Block {i}: {M_np[i*6+3, i*6+3]:.4f}")
        # ===========================================================
